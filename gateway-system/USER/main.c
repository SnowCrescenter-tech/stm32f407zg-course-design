...
void control_task(void *pdata)
{
    ...
    while (1)
    {
        if (Menu_State != 2) // 在 待机(0) 和 导航(1) 状态下，使用信号量阻塞等待外部中断（按一下响应一次，省电防连跳）
        {
            ...
            // 立即硬件级屏蔽这四个按键的 EXTI 中断触发，以供软件防抖
        ...
        {
            u8 pin_key = 0; // 0=未按, 1=WK_UP(加), 2=KEY1(减), 3=KEY0(确定), 4=KEY2(回退)
            ...
            if (pin_key > 0)
            {
                ...
                else if (Menu_Cursor == 4) // 编辑 Crash Sim
                {
                    if (pin_key == 1 || pin_key == 3) // WK_UP (+) 或 KEY0 (确定)
                    {
                        u16 menu_w = 320;
                        u16 menu_h = 250;
                        u16 menu_x = (lcddev.width - menu_w) / 2;
                        u16 menu_y = (lcddev.height - menu_h) / 2;
    ...
    while (1)
    {
        ...
        if (err == OS_ERR_NONE && data != NULL)
        {
            ...
            else
            {
                ...
                {
                    if (UI_Mode == 0) // 折线图看板 (归一化双曲线双轴刻度版)
                    {
                        u16 grid_x = 55;
                        u16 grid_w = lcddev.width - 110;
                        u16 grid_y = 40;
                        u16 grid_h = lcddev.height - 200;
                    ...
                    else if (UI_Mode == 1) // 仪表盘看板 (4通道大图表盘)
                    {
                        u16 bar_w = lcddev.width - 80;
                        POINT_COLOR = WHITE;
                        BACK_COLOR = BLACK;
                        ...
                        // 尾部增加空格，并且将颜色由暗沉的蓝色(BLUE)更换为极其亮眼的青色(CYAN)
                    ...
                    else if (UI_Mode == 2) // 日志看板 (Terminal 命令行控制台风格)
                    {
                        u16 box_x = 30;
                        u16 box_w = lcddev.width - 60;
                        u16 box_y = 40;
                        u16 box_h = lcddev.height - 230;
                        int draw_y;
                        ...
                        {
                            u8 show_cnt = temp_log_count > 6 ? 6 : temp_log_count;
                            u8 start_i = temp_log_count > 6 ? (temp_log_count - 6) : 0;
                ...
                if (menu_s > 0)
                {
                    ...
                    if (menu_s == 2 && (menu_c == 0 || menu_c == 1))
                    {
                        ...
                        if (menu_c == 0) // 温度范围为 15.0 ~ 50.0 C (150 ~ 500)
                        {
                            ...
                            // 红色填充代表温度
                            if (fill_h > 0) LCD_Fill(bar_x + 1, bar_y + bar_h - fill_h - 1, bar_x + bar_w - 1, bar_y + bar_h - 1, RED);
                        ...
                        else // 光照范围为 10 ~ 95
                        {
                            ...
                            // 黄色填充代表光照
                            if (fill_h > 0) LCD_Fill(bar_x + 1, bar_y + bar_h - fill_h - 1, bar_x + bar_w - 1, bar_y + bar_h - 1, YELLOW);
                            if (fill_h < (bar_h - 2)) LCD_Fill(bar_x + 1, bar_y + 1, bar_x + bar_w - 1, bar_y + bar_h - fill_h - 2, DARKBLUE);
                        }
                    }
                }
